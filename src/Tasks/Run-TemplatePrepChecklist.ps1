...
try {
    ...
    Write-Host "  1) SDelete with -z on the template drive"
    Write-Host "  2) Sysprep (Generalize + OOBE + Shutdown)"
    Write-Host "  3) (Optional) Sparsify/convert image"
    Write-Host "  4) Convert VM to template"
    Write-Host ""
    ...
    if (-not (Test-Path $sysprepExePath)) {
        ...
        $openSysprep = Read-Host "Open Sysprep now (final step before template conversion)? (Y/N)"
    ...
    Write-Host "If you skip Sysprep, shut down the VM before converting to template in Proxmox." -ForegroundColor DarkGray
}
catch {
